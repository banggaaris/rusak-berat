...
const SAKTI_API = 'https://fusesakti.kemenkeu.go.id/sakti/backend/asset-tetap/assetTrx/deletePengembangan';
...
export async function DELETE({ request }) {
    try {
        ...
        if (!object) {
            return new Response(JSON.stringify({ error: 'Missing object ID' }), {
                status: 400,
                headers: { 'Content-Type': 'application/json' }
            });
        }
        ...
        const response = await fetch(SAKTI_API, {
            method: 'DELETE',
            headers: headers,
            body: JSON.stringify({ object: Number(object) })
        });
